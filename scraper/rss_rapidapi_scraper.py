...
COUNTRY_CODES = env_list('RSS_JOB_COUNTRY_CODES', [
    'es',
    'mx',
    'co',
    'cl',
    ...
    for country_code in COUNTRY_CODES:
        ...
        if status != 200:
            ...
            if status in STOP_STATUSES:
                print('RapidAPI RSS jobs: stopping to protect quota/auth after quota or auth response')
                return jobs
            continue
        ...
        for item in items:
            ...
            location = pick(item, 'location', 'job_location', 'formatted_location', 'city', 'country') or country_code
            description = pick(item, 'description', 'job_description', 'summary', 'snippet', 'content')
            job_url = pick(item, 'url', 'job_url', 'link', 'apply_url', 'application_url')
            posted_at = pick(item, 'posted_at', 'posted_date', 'date_posted', 'published', 'pubDate', 'created_at')
            job_type = pick(item, 'employment_type', 'job_type', 'type') or 'FULL_TIME'
            salary = pick(item, 'salary', 'salary_range', 'compensation', 'salary_min', 'salaryMax')
...
if __name__ == '__main__':
    ...
    new_jobs = scrape_rss_rapidapi()
    output = write_output(existing, new_jobs)
    print(f'RapidAPI RSS jobs merged: +{len(new_jobs)} raw jobs, {output["totalJobs"]} total jobs')
